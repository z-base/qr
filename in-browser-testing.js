...
function display(value) {
  ...
  const svgText = qr_default(value, "svg");
...
function print(value) {
  ...
  const svg = qr_default(value, "svg");
...
async function scan() {
  if (!await qr_scanner_min_default.hasCamera())
  ...
  return new Promise((resolve, reject) => {
    let settled = false;
    const scanner = new qr_scanner_min_default(video, (result) => {
      if (settled)
        return;
      settled = true;
      cleanup();
